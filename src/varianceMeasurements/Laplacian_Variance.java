...
public class Laplacian_Variance implements PlugIn {
	...
	double getPhaseVariance(int n, String type, boolean normalise) throws Throwable {
		...
		int[] neighbours = new int[] {n-area, n-width, n-1, n+area, n+width, n+1};
		
		if ( type.equals("Exact") ) {
			for (int i: neighbours) phaseVariance += exact.get(i) / factor;
			phaseVariance -= 6*exact.get(n) / factor;
		} else if ( type.equals("Unwrap") ) {
			for (int i: neighbours) phaseVariance += unwrap.get(i) / factor;
			phaseVariance -= 6*unwrap.get(n) / factor;
		}
