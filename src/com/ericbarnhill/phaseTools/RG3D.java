...
public class RG3D { 
	...
	Preferences prefs; // for differences file
	...
	private void initVariablesArray() {
		differences = Differences3D.getDifferences();
        width = phase.length;
        ...
        for (int z = 0; z < depth; z++) {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    float magValue = (float)magnitude[x][y][z];
                    magnitudes.add(magValue);
                    float value = (float)phase[x][y][z];
...
void calculateVoxelReliabilitiesMagnitude() {
    ...
    for (Voxel v : voxels) {
        if (v.value == v.value) {
            v.reliability = getReliability(voxelNumber)/(magnitudes.get(voxelNumber)*magnitudes.get(voxelNumber));
...
float getReliability(int voxelNumber) {
    ...
    for (int d = 0; d < differences.length; d = d + 2) {
        ...
        int address1 = x1 + y1*width + z1*area;
        int address2 = x2 + y2*width + z2*area;
...
void mergeGroups(){
	...
	double[][][] getArray() {
		...
		double[][][] unwrap = new double[width][height][depth];

        for (int z = 0; z < depth; z++) {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int voxelNumber = x + y*width + z*area;
	...
	public double[][][] unwrapPhase(double[][][] phase, double[][][] magnitude) {
		return unwrapPhase(phase, magnitude, 3.14);
	}
    ...
    public static void main(String[] args) {
        ...
        if (as.paths[2].endsWith("nii") || as.paths[2].endsWith(".nii.gz")) {
            ...
            magnitude = ArrayMath.shiftDimR(magnitude);
            magnitude = ArrayMath.normalize(ArrayMath.medianFilter3d(magnitude,2));
            for (int n = 0; n < fl; n++) { //to be parallelized
                phase[n] = rg3d.unwrapPhase(phase[n], magnitude[n], 3.14);
            }
        } else {
            for (int n = 0; n < fl; n++) { //to be parallelized
                phase[n] = rg3d.unwrapPhase(phase[n], 3.14);
            }
        }
        phase = ArrayMath.shiftDimL(phase);
